##################### Mega survey camera traps: formatting but no species filtering ###
# Date: 1-29-24
# updated: 2-16-24: keep all species rather than filtering
...
# this was in other version of the code; not using for this version
...
# never mind, we are going to do this the hard way this time
...
# but again, why make things easier?
...
pila_xwalk$adjusted <- c('Common Opposum' ,"Dice's Cottontail",'Central American Red Brocket','tourist',
                         'Spotted Paca','setup','Great Curassow','Collared Peccary','Puma','squirrel_uid','Coyote','Crested Guan',
                         'Mexican Hairy Dwarf Porcupine','Spotted Wood-quail','Striped Hog-nosed Skunk','tinamou_highland','ranger','Jaguar',
                         'White-nosed Coati','Tayra','guan-black','Ocelot','small_bird_uid',"Baird's Tapir",'small_mammal_uid','Chiriqui Quail-dove','Central American Agouti','end',
...
test[7,] <- c('Common Oppossum','opossum_common','Mammalia','Didelphimorphia','Didelphidae','Didelphis','marsupialis')
test[10,] <- c("Dice's Cottontail","cottontail_dices",'Mammalia','Lagomorpha','Leporidae','Sylvilagus','dicei')
test[14,] <- c("Jaguar",'jaguar','Mammalia','Carnivora','Felidae','Panthera','onca')
test[18,] <- c("Northern Olingo",'olingo_northern','Mammalia','Carnivora','Procyonidae','Bassaricyon','gabbii')
test[20,] <- c("Oncilla",'oncilla','Mammalia','Carnivora','Felidae','Leopardus','tigrinus')
...
#write.csv(img_combined, "Data/spatial/CameraTraps/megasurvey/images_megasurvey_WI_format_unfiltered.csv", row.names=F)
...
#write.csv(img_combined2, "Data/spatial/CameraTraps/megasurvey/images_megasurvey_WI_format_wPowershell_unfiltered.csv", row.names=F)
